...
class DenseNet264(nn.Module):
    """
    Implementation of the DenseNet264 architecture.
    ...
    def __init__(self, in_channels=3, num_classes=1000, growth_rate=32, compression_factor=0.5):
        ...
        self.denseblock3 = DenseBlock(in_channels=input_features, num_layers=64, growth_rate=growth_rate)
        input_features = input_features + growth_rate * 64
        self.transitionblock3 = TransitionLayer(in_channels=input_features, compression_factor=compression_factor)
        input_features = int(input_features * compression_factor)

        self.denseblock4 = DenseBlock(in_channels=input_features, num_layers=48, growth_rate=growth_rate)
        input_features = input_features + growth_rate * 48
    ...
    def forward(self, x):
        """
        Forward pass for the DenseNet264 model.
