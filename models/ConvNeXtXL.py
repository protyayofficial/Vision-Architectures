...
class ConvNeXtXL(nn.Module):
    """
    ConvNeXt-XL architecture for image classification. It consists of a stem convolution
    ...
    Args:
        ...
        channel_config (list): List containing the number of channels at each stage. 
                               Default is [256, 512, 1024, 2048].
        repeat_config (list): List containing the number of ConvNeXt blocks to repeat at each stage. 
                              Default is [3, 3, 27, 3].
    """
    def __init__(self, in_channels=3, num_classes=1000, channel_config=[256, 512, 1024, 2048], repeat_config=[3, 3, 27, 3]):
        super().__init__()
    ...
    def forward(self, x):
        """
        Forward pass of the ConvNeXtXL model.
